...
2. When classification completes at `intent_classification`, exactly one class and confidence are written at `intent_classification`.
...
3. When the gate denies at `send_gate_decision`, the next owner action is blocked at `engagement_attempt`. idempotent
...
5. When the resume window matures at `auto_reply_pause`, re-entry eligibility rechecks at `send_gate_decision`.
...
2. When the sequence instance activates at `sequence_instance`, a single channel owner is assigned at `engagement_attempt`.
3. When another owner schedule is requested while one owner is active at `engagement_attempt`, the request is rejected at `engagement_attempt`. idempotent
...
**Path:**
1. When a forward owner order is authored at `sequence_rule_set`, ordered owner state is attached at `sequence_instance`.
...
4. When an inbound reply arrives during the retry cycle at `conversation_thread`, the nested owner context updates at `conversation_thread`.
...
3. When the pending job is nullified at `engagement_attempt`, the scheduled send is canceled at `attempt_job_queue`.
...
5. When calendar phase or owner attempt state changes at `campaign_calendar` or `engagement_attempt`, distinct chronology entries occur at `engagement_record`.
**Nodes touched:**
- `campaign_calendar`
- `sequence_enrollment`
- `sequence_instance`
...
**Path:**
1. When runtime escalation opens at `escalation_ticket`, the item is typed as engagement-escalation at `escalation_ticket`.
2. When engagement-escalation typing occurs at `escalation_ticket`, support context is assembled at `support_context_bundle`.
3. When support context is assembled at `support_context_bundle`, thread linkage is attached at `conversation_thread`.
4. When the ticket and context are ready at `escalation_ticket`, ticket-opened chronology occurs at `engagement_record`.
5. When the escalation item is worked at `escalation_ticket`, the same item remains the support queue item at `escalation_ticket`. idempotent
**Nodes touched:**
- `escalation_ticket`
- `support_context_bundle`
- `conversation_thread`
...
2. When reason and priority occur at `escalation_ticket`, reply and thread context are gathered from `conversation_thread`.
...
2. When intent or escalation state changes at `intent_classification` or `escalation_ticket`, the thread work state updates at `conversation_thread`.
...
6. When the context projection completes at `triage_context_snapshot`, visible triage context changes at `conversation_thread`.
**Nodes touched:**
- `conversation_thread`
- `triage_context_snapshot`
- `intent_classification`
...
4. When the posture is Active at `firm_operations_bind`, contact-facing send eligibility can occur at `send_gate_decision`.
...
6. When an inbound needs human review while not Active at `conversation_thread`, escalation can still open at `escalation_ticket`.
