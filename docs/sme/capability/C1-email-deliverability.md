...
**implementation:**  
On Sending infrastructure, you can now see DMARC status (record present, policy ≥ p=none, rua configured, alignment pass) per From domain.  
...
**implementation:**  
On Sending infrastructure → Warmup, you can now see the ramp plan (daily unique recipients + absolute volume) for each new domain/IP.  
...
**implementation:**  
On Sending infrastructure, you can now see Postmaster / FBL registration status per sending domain.  
...
**implementation:**  
On Sending infrastructure, you can now set the firm’s From identity to the authenticated branded subdomain (display name = firm).  
...
**implementation:**  
On Sending infrastructure, you can now see Return-Path / custom MAIL FROM on a platform bounce subdomain (SPF-authenticated) separate from firm header From.  
...
**implementation:**  
On Sending infrastructure, you can now see IP tier (shared pool vs dedicated) with promotion criteria: sustained volume + clean complaint/bounce.  
