...
type ChronologyRow = {
  ...
  title: string;
  detail: string;
  kind: "touch" | "halt" | "reply" | "system";
};

const DEMO_CHRONOLOGY: Record<string, ChronologyRow[]> = {
  sarah: [
    {
      id: "h1",
      at: "Today · 09:14",
      title: "Permit expiry trigger",
      detail: "Activation sequence armed under bound pack.",
      kind: "system",
    },
    {
      id: "t1",
      at: "Yesterday · 16:02",
      title: "Firm → client email",
      detail: "Eligibility refresh · opened",
      kind: "touch",
    },
    {
      id: "r1",
      at: "14 May · 11:20",
      title: "Contact reply",
      detail: "SMS · preferred channel confirmed",
      kind: "reply",
    },
  ],
  default: [
    {
      id: "t0",
      at: "Mon · 10:00",
      title: "Firm → client touch",
      detail: "Routine check · delivered",
      kind: "touch",
    },
  ],
};
      ...
      ? [
          {
            id: "halt-live",
            at: halt.at,
            title: `Halt outreach · ${halt.scope === "book" ? "Firm book" : "This contact"}`,
            detail: halt.reason.trim() ? halt.reason : "No reason recorded",
            kind: "halt" as const,
          },
        ]
      : []),
    ...(DEMO_CHRONOLOGY[clientId] ?? DEMO_CHRONOLOGY.default),
  ];

  useEffect(() => {
    if (!focusLabel) return;
    if (focusLabel === "Engagement record") setTab("engagement");
    if (focusLabel === "Client workspace" || focusLabel === "Client Brief") setTab("brief");
  }, [focusLabel, focusSeq]);

  function exportChronology() {
    const text = rows.map((r) => `${r.at}\t${r.title}\t${r.detail}`).join("\n");
    ...
    >
      ...
      {tab === "brief" ? (
        ...
        >
          ...
          >
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: t.textPrimary }}>Engagement record</div>
              <div style={{ fontSize: 11, color: t.textMuted, marginTop: 2 }}>
                Newest first · chronology only — no authorship
              </div>
            </div>
            <button
              type="button"
              onClick={exportChronology}
              style={{ ...secondaryControlStyle(t), padding: "5px 10px", fontSize: 11 }}
              title="Export visible chronology rows"
            >
              {exported ? "Copied" : "Export chronology"}
            </button>
          </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: open ? t.accentBg : t.bgSecondary,
                    cursor: "pointer",
                    fontFamily: "inherit",
                  }}
                >
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: t.textPrimary }}>{row.title}</span>
                    <span style={{ fontSize: 10, color: t.textDim }}>{row.at}</span>
                  </div>
                  {open ? (
          ...
          {halt ? (
            <p style={{ margin: "14px 0 0", fontSize: 11, color: t.textDim }}>
              Halt events stay on this chronology for firm-desk memory under your license.
            </p>
          ) : null}

          <div style={{ marginTop: 14 }}>
            <button type="button" onClick={onHaltOutreach} style={primaryControlStyle(t)}>
              Halt outreach
            </button>
          </div>
        </div>
