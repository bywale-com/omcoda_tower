...
export function BoardModule({
  ...
  const columns: ColumnsType<BoardRow> = [
    {
      ...
      render: (name, row) => (
        <Surface label="Client row">
          <Space>
            <Text strong={row.id === activeClientId}>{name}</Text>
            {row.halted ? <Tag color="error">Halted</Tag> : null}
          </Space>
        </Surface>
      ),
    },
    ...
    { title: "Runtime", dataIndex: "runtime", key: "runtime", width: 90 },
    { title: "Reachability", dataIndex: "reachability", key: "reachability", width: 110 },
    { title: "Last touch", dataIndex: "lastTouch", key: "lastTouch", width: 100 },
  ];
  ...
  return (
    <div style={{ display: "flex", height: "100%", minHeight: 0 }}>
      <div style={{ width: 420, flexShrink: 0, display: "flex", flexDirection: "column", minHeight: 0, borderRight: "1px solid var(--ant-color-split)" }}>
          ...
          <Space direction="vertical" size="middle" style={{ width: "100%" }}>
            ...
            {bookHalted || bookHalt ? (
              <Space>
                ...
                <Button size="small" onClick={onResumeBook}>Resume book</Button>
              </Space>
            ) : (
              <Surface label="Halt outreach">
                <Button size="small" danger onClick={onHaltBook}>Halt firm book</Button>
              </Surface>
            )}
            ...
            {pendingHard.length > 0 ? (
              <Space wrap>
                {pendingHard.map((label) => (
                  <Tag key={label} color="warning">{label} pending</Tag>
                ))}
              </Space>
            ) : null}
