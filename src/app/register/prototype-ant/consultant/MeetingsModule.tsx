...
export function MeetingsModule({
  ...
  const listColumns: ColumnsType<LiveBriefMeeting> = [
    {
      ...
      render: (name, row) => (
        <Surface label="Meeting row">
          <Space>
            <Text strong={row.id === selectedId}>{name}</Text>
            <Tag color={row.status === "Tentative" ? "default" : "processing"}>{row.status}</Tag>
          </Space>
        </Surface>
      ),
    },
    { title: "Time", dataIndex: "time", key: "time", width: 130 },
    {
      title: "Phase",
      dataIndex: "phase",
      key: "phase",
  ...
  return (
    <Surface
      label="Meetings"
      style={{ display: "flex", height: "100%", minHeight: 0, gap: 12, padding: 12 }}
    >
      <div
        style={{
          width: selected ? 360 : "100%",
          maxWidth: selected ? 400 : undefined,
          flexShrink: 0,
          display: "flex",
                ...
                {["Thu", "Fri", "Next week"].map((day) => {
                  ...
                  return (
                    <div key={day}>
                      ...
                      {dayRows.length === 0 ? (
                        <Text type="secondary" style={{ display: "block", padding: "6px 0" }}>—</Text>
                      ) : (
                        dayRows.map((row) => (
                          <Button
                            key={row.id}
      ...
      {selected ? (
        ...
        >
          <div style={{ padding: "12px 16px", borderBottom: "1px solid var(--ant-color-split)" }}>
            ...
            <Text type="secondary">{selected.time} · {selected.phase}</Text>
          </div>
          ...
          >
            <Space direction="vertical" size="middle" style={{ width: "100%" }}>
              <Space style={{ width: "100%", justifyContent: "space-between" }} wrap>
                ...
                <Space>
                  {selected.asOf ? (
                    <Text type="secondary" style={{ fontSize: 11 }}>{selected.asOf}</Text>
                  ) : null}
              ...
              <div>
                <Title level={5} style={{ margin: 0 }}>{selected.contactName}</Title>
                <Text type="secondary">{selected.purpose}</Text>
                {selected.highlight ? (
                  <Tag color="processing" style={{ marginTop: 8 }}>{selected.highlight}</Tag>
                ) : null}
              </div>
                  ...
                  <Table
                    ...
                    columns={[
                      ...
                      { title: "Value", dataIndex: "value", key: "value" },
                      {
                        ...
                        key: "signal",
                        render: (s, row) => (
                          <Tag title={row.signalHint ?? s}>{s}</Tag>
                        ),
                      },
                    ]}
                  />
