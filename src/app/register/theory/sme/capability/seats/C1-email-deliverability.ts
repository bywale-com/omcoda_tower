  ...
  items: [
    {
      ...
      implementation: "On Sending infrastructure, open a sending-domain row; the Authentication panel shows an SPF status row with ESP-include and return-path chips before the domain may be bound for volume.  \nOn Firm operations bind firm detail → Send gates panel, the Domain authentication readiness row shows an SPF-not-ready deny chip blocking Armed / Active volume until SPF passes.",
    ...
    },
    {
      ...
      implementation: "On Sending infrastructure, open the firm's sending identity row and click Generate DKIM keys; the Authentication panel shows signing status chips for aligned d= and From domain.  \nOn Firm operations bind firm detail → Send gates panel, the DKIM-aligned identity readiness row must pass before Opt-in message or Nudge message CEM can leave.",
    ...
    },
    {
      ...
      implementation: "On Sending infrastructure, open the From-domain row; the Authentication panel shows DMARC status chips for record present, policy ≥ p=none, rua configured, and alignment pass.  \nOn Activation state Progress, the ready-to-send checklist row stays closed until DMARC readiness is green for the bound sending identity.",
    ...
    },
    {
      ...
      implementation: "On Sending infrastructure → Warmup, the Warmup plan panel shows a ramp row per new domain/IP with daily unique-recipient cap, absolute-volume cap, and current stage status.  \nOn Activation state Progress, the ready-to-send checklist requires Warmup stage complete or in-band capped status — cold full-book blast stays blocked.",
    ...
    },
    {
      ...
      implementation: "On Activation state Progress, the readiness checklist shows deliverability rows beside commercial/consent rows: auth green, Warmup stage ok for planned volume, and reputation unit not quarantined.  \nOn Firm operations bind firm detail → Send gates panel, Armed / Active readiness rows require those green status chips — consent-ready alone does not open volume.",
    ...
    },
    {
      ...
      implementation: "On Firm operations bind firm detail → Send gates panel, the Warmup / throttle remaining row shows capacity chips for the bound sending identity; enrollment beyond the cap queues or blocks.\nOn Sending infrastructure → Warmup, the consumed-vs-allowed volume row shows today's enforced budget — sequencer and ESP share one counter.",
    ...
    },
    {
      ...
      implementation: "On Sending infrastructure → Warmup, re-warmup trigger rows name domain change, IP tier change, ESP migration, or idle-beyond-policy as the reason and show the active ramp status.  \nOn Activation state Progress and Firm operations bind firm detail → Send gates panel, tightened-cap rows stay active until re-warmup completes — prior volume rights do not auto-restore.",
    ...
    },
    {
      ...
      implementation: "On Sending infrastructure, the Delivery event ingest panel shows rows for accepted, deferred, hard/soft bounce, complaint, delivered, and rejected, with linked updates to Warmup, Suppression list, and reputation-unit health.  \nOn Audit trail, open deliverability Change event rows to see why Firm operations bind firm detail → Send gates panel blocked or capped a firm.",
    ...
    },
    {
      ...
      implementation: "On Firm operations bind firm detail → Send gates panel, ordered deny chips render in row order: CASL silence/consent basis, then Suppression list (per-tenancy + global).\nOn Suppression list (per-tenancy), opt-out / silence, complaint, and hard-bounce rows appear as technical denies alongside Book readiness permission-to-send.",
    ...
    },
    {
      ...
      implementation: "On Book readiness Verdict list and Firm health Sequence health, contact rows show sunsetting / chronic non-engagement flags that block or require re-permission before reactivation volume.  \nOn Firm operations bind firm detail → Send gates panel, reactivation bursts show sunsetting-suppressed deny chips and cannot enroll those addresses without a new engagement path.",
    ...
    },
    {
      ...
      implementation: "On Sending infrastructure, the From identity dropdown on the sending-identity row offers only authenticated branded subdomains with display name = firm.  \nOn Opt-in message and Nudge message, the From header row shows that identity; platform shared From is not offered for firm-branded CEMs.",
    ...
    },
    {
      ...
      implementation: "On Sending infrastructure, the Envelope panel shows Return-Path / custom MAIL FROM as a platform bounce-subdomain row with SPF-authenticated status separate from firm header From.  \nOn Audit trail deliverability event rows, bounce correlation cites that Return-Path so Suppression list updates reliably.",
    ...
    },
    {
      ...
      implementation: "On Sending infrastructure, the IP tier row shows shared pool vs dedicated with promotion-criteria chips for sustained volume and clean complaint/bounce.  \nOn Warmup, dedicated IP assignment starts its own ramp row; PTR/rDNS hygiene status must be green before dedicated volume is allowed.",
