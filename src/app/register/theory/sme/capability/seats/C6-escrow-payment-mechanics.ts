  ...
  items: [
    {
      ...
      implementation: "On Commercial, on Escrow status, you can now open the firm’s Escrow ledger instruments (held / release_pending_window / released / returned / forfeited / disputed) with principal, currency, and terms_version_id.  \nOn Release control, you can now act only against a ledger instrument — never against an orphan provider charge.",
    ...
    },
    {
      ...
      implementation: "On Escrow status, you can now see only the closed status set and the legal next transitions for the instrument.  \nOn Release control, you can now attempt a transition; illegal edges fail closed and write a rejected transition receipt.",
    ...
    },
    {
      ...
      implementation: "On Escrow status, you can now see Held only when provider_ref confirms immobilized funds; otherwise status stays pending_accept / failed_hold.  \nOn Activation state, on Progress, you can now treat escrow hard-input satisfied only after ledger reaches `held`.",
    ...
    },
    {
      ...
      implementation: "On Commercial, you can now see Rail posture: Connect marketplace hold–release (firm payer) with provider account / charge / transfer refs on the instrument.  \nOn Release control, you can now initiate provider release/transfer only from `release_pending_window` after evidence passes — not from Accept.",
    ...
    },
    {
      ...
      implementation: "On Escrow status, you can now see Held principal, Release amount / schedule, Hard cap, and Remaining releasable.  \nOn Release control, you can now fail a release that would exceed Remaining releasable — receipt shows cap_block.",
    ...
    },
    {
      ...
      implementation: "On Escrow status, you can now see Hold confirmation sourced from provider webhook (event id stored once).  \nOn Commercial, you can now replay webhook application safely — duplicate event ids are no-ops.",
    ...
    },
    {
      ...
      implementation: "On Release control, you can now reject verification events lacking enrollment↔meeting linkage (reason: attribution_failed).  \nOn Escrow status, you can now see Attribution path (campaign / enrollment / contact / meeting) or Attribution failed.",
    ...
    },
    {
      ...
      implementation: "On Release control, before transfer, you can now open Release evidence package (required fields validated).  \nOn Escrow status, when status is release_pending_window or released, you can now inspect the same package.",
    ...
    },
    {
      ...
      implementation: "On Release control, you can now Execute release (provider transfer) for a releasable instrument; success writes transfer_ref and status `released`.  \nOn Escrow status, you can now refuse manual “mark released” without transfer_ref (fail closed).",
    ...
    },
    {
      ...
      implementation: "On Release control (or Return control adjacent), you can now Execute return for instruments meeting return predicates (abandoned / never-running / window_abort per frozen terms).  \nOn Escrow status, you can now see `returned` with refund_ref and return_reason — not a silent dashboard refund.",
    ...
    },
    {
      ...
      implementation: "On Accept terms / Escrow terms, you can now complete firm payment method entry through provider tokenization only.  \nOn Escrow status / ledger, you can now store provider customer and payment_method refs — never PAN / CVV.",
    ...
    },
    {
      ...
      implementation: "On Commercial, on Escrow status, you can now oversee all instruments for the firm: status, principal, remaining releasable, window, evidence summary, last commercial receipt, dispute flag, counsel_gate.  \nOn Release control, you can now act from that same read model.  \nOn Customer support, on Support context, you can now pull the same Commercial facts — never Meta billing objects.",
