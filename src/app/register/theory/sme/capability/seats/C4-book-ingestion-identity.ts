...
export const seat: SmeSeat = {
  ...
  items: [
    ...
    },
    {
      ...
      implementation: "On Imports, the Ingestion-complete status chip turns on only after Normalized preview, Match keys, and saved Field→Q-ID map version are all complete.  \nOn Book readiness → Audits, Start Audit run is enabled only for Ingestion-complete batches; Verdict list shows validation-class pending when checks are not wired.",
    ...
    },
    {
      ...
      implementation: "On Authorize book hand-over into Imports, the Sync posture chip reads One-way land only / Incremental sync deferred (KU #7); no V1 sync-complete button appears beside Field→Q-ID map.  \nOn Activation state → Progress, running can open without sync-complete; currency uses re-hand-over upsert until CDC ships.",
    ...
    },
    {
      ...
      implementation: "On Authorize book / Imports, any future live-crm pull is labeled as feeding the same pipeline cards: Source header inventory → Normalized preview → Field→Q-ID map → Match keys → Quarantine; V1 shows Vertical continuous pull deferred (KU #7).  \nOn Book readiness → Audit run, the batch picker uses the same Audit contract for file and future pull; Verdict list has no pull-specific bypass.",
